...
func (r *renderOpts) Run() error {
	...
	if len(renderConfig.ClusterCIDR) > 0 {
		...
		if !anyIPv4 {
			// Single-stack IPv6 cluster, so listen on IPv6 not IPv4. IPv6
			// syntax requires brackets around the address part to distinguish
			// it from the port, which then means that we also need quotes
			// around the whole thing to avoid confusing the YAML parser.
